...
public class CsvReaderUnitTest {
    ...
    public Stream<DynamicTest> scenarios() {
        return Stream.of(
                ...
                // Comments
                new Assertion("The whole document can be a comment", "#", list()),
                new Assertion("The whole document can be a comment (plus newline)", "#\n", list()),
                new Assertion("Commas after comment do not create new fields (doc start)", "#, a, b", list()),
                new Assertion("Commas after comment do not create new fields (line start)", "cell 1\n#, a, b", list(list("cell 1"))),
                new Assertion("Whitespace before comment is significant", "cell 1 # a, b", list(list("cell 1 "))),
                new Assertion("Whitespace before comment and after comma creates a blank field", "cell 1, # a, b", list(list("cell 1", " "))),
                // bug: new Assertion("Quoted values can contain comment character at start", "\"# cell\"", list(list("# cell"))),
                // bug: new Assertion("Quoted values can contain comment character at end", "\"cell #\"", list(list("cell #"))),
                // bug: new Assertion("Quoted values can contain comment character in middle", "\"cell # cell\"", list(list("cell # cell"))),
                // bug: new Assertion("Comment character can start the next line of a multiline quoted value", "\"line 1\n# line 2\"", list(list("line 1\n# line 2"))),
                // bug: new Assertion("Comment character cab appear on the next line of a multiline quoted value", "\"line 1\nline 2 #\"", list(list("line 1\nline 2 #"))),

                null
        ).filter(Objects::nonNull).map(it -> DynamicTest.dynamicTest(it.displayName, () -> {
            final List<List<String>> output;
            try (StringReader stringReader = new StringReader(it.input); CsvReader csvReader = new CsvReader(stringReader)) {
                output = csvReader.readFile(false);
            }
            Assertions.assertEquals(it.expectedOutput, output);
        }));
    }
    ...
    public void testReadFile() throws IOException, CsvException {
        ...
        try (final CsvReader csvReader = new CsvReader(new StringReader(stringBuilder.toString()))) {
            ...
            Assertions.assertEquals(result.stream().filter(Objects::isNull).count(), 0, "Some results were null.");
            Assertions.assertEquals(result.stream().mapToLong(Collection::size).sum(), 64, "Wrong number of total fields returned.");
        }
    }
    ...
    private static StringBuilder initStrings() {
        ...
        stringBuilder.append(StringEscapeUtils.escapeCsv("tricky\"\"input")).append(",quotes\n");
        stringBuilder.append(StringEscapeUtils.escapeCsv("tricky\",\"input")).append(",quotes\n");
        stringBuilder.append(StringEscapeUtils.escapeCsv("\"aꜬbꜢd\uD801\uDF22e∛f\"")).append(",quotes\n");
        stringBuilder.append("# comment line, with \" many # odd ,\"'\"\" characters in.\n");
        stringBuilder.append("# comment line, with \" many # odd ,\"'\"\" characters in.\n");
        stringBuilder.append("\"\"\"\"\n");
    ...
    public void testParseLine() throws IOException, CsvException {
        ...
        try (final CsvReader csvReader = new CsvReader(new StringReader(stringBuilder.toString()))) {
            ...
            Assertions.assertEquals(line9.get(0), "tricky'input", "Apostrophe (') character was mis-interpreted.");
            ...
            Assertions.assertEquals(line10.get(0), "tricky\"input", "Interstitial quotation mark (\") was mis-interpreted.");
            ...
            Assertions.assertEquals(line11.get(0), "tricky,input", "Interstitial comma (,) was mis-interpreted.");
            ...
            Assertions.assertEquals(line12.get(0), "tricky\"\"input", "Pair of quotation marks (\"\") was mis-interpreted.");
            ...
            Assertions.assertEquals(line13.get(0), "tricky\",\"input", "Sequence \",\" was mis-interpreted.");
            ...
            Assertions.assertEquals(line14.get(0), "aꜬbꜢd\uD801\uDF22e∛f", "Quoted unicode characters caused problems.");
            ...
            Assertions.assertEquals(line15.get(0), "\"", "The raw field \"\"\"\" didn't get unescaped to a single \" character.");
            ...
            Assertions.assertEquals(line16.get(0), "bob \"wonderful\" smith", "An interstitially quoted string was mis-interpreted.");
            ...
            Assertions.assertEquals(line17.get(0), "0", "Normal single-character string was not pulled out successfully.");
            Assertions.assertNotNull(line17.get(3), "String with many quotations was null.");
            Assertions.assertEquals(line17.get(3), "\"AAAAAAAAAA\": BB BB BB 'ABCDEFG QWEIR', 1234-5678", "Interstitial quotation marks at the beginning of the string caused an issue.");
            ...
            Assertions.assertEquals(line18.get(0), "333333333", "Made-up stucode was not pulled out successfully.");
            Assertions.assertNotNull(line18.get(27), "String with many quotations was null.");
            Assertions.assertEquals(line18.get(27), "\"NNNNNNNNNNNN AMBNDJ\": AMAMAMMAMAMA 'QPQPQPQPQP', 1234-5678", "Interstitial quotation marks at the beginning of the string caused an issue.");
